...
var jampack = function () {
  ...
  $(document).on(
    "click",
    ".selectable-dropdown .dropdown-menu .dropdown-item",
    function (e) {
  ...
  $(document).on(
    "click",
    ".selectable-split-dropdown .dropdown-menu .dropdown-item",
    function (e) {
      var selText = $(this).text(),
        selbg = $(this).attr("data-color");
      $(this)
        .parents(".selectable-split-dropdown")
        .find(".btn-dyn-text")
        .html(selText);
    }
  );
  ...
  $(document).on("click", ".drawer-toggle-link", function (e) {
    ...
    if ($(this).attr("data-drawer") == "push-normal") {
      ...
      if ($(targetDrawer).hasClass("drawer-left"))
        $wrapper.addClass(
          "hk-drawer-push hk-drawer-wth-nav-push hk-drawer-pushleft"
        );
      else
        $wrapper.addClass(
          "hk-drawer-push hk-drawer-wth-nav-push hk-drawer-pushright"
        );
  ...
  $(document).on("input", ".password-check input", function (e) {
    if ($(this).val() == "") {
      ...
      $(this)
        .parent()
        .find(".input-suffix > span:first-child")
        .removeClass("d-none");
      $(this)
        .parent()
        .find(".input-suffix > span:last-child")
        .addClass("d-none");
    }
  });
};
...
var chatApp = function () {
  ...
  if ($(".chatapp-wrap #chat_body").length > 0) {
    var demoScroll = document.querySelector(
      "#chat_body .simplebar-content-wrapper"
    );
    demoScroll.scrollTo({ top: 100000, behavior: "smooth" });
    $(document).on(
      "click",
      ".chat-contacts-list > .list-group-item > div.media",
      function (e) {
        $(".chat-contacts-list > .list-group-item > div.media").removeClass(
          "active-user"
        );
        ...
        $(".chatapp-single-chat header .media-head .avatar").replaceWith(
          $(this).find(".media-head").html()
        );
        if (!$(".chat-contacts-list").hasClass("group-list")) {
          $(".chatapp-single-chat #dummy_avatar >li > .avatar").replaceWith(
            $(this).find(".media-head").html()
          );
          $(".chatapp-single-chat #dummy_avatar >li > .avatar")
            .removeClass(
              "avatar-sm badge-on-avatar badge-bottom-right custom-badge-on-avatar"
            )
            .addClass("avatar-xs")
        ...
        $(
          ...
          .remove();
        $(
          "#audio_call .modal-body .avatar,#video_call .modal-body .avatar"
        ).addClass("d-20");
        $(".chat-info .cp-name")
          .next()
          .text($(this).find(".user-status").html());
  ...
  if ($(".hk-chat-popup .chat-popup-body").length > 0) {
    var demoScroll = document.querySelector(
      ".chat-popup-body .simplebar-content-wrapper"
    );
    demoScroll.scrollTo({ top: 100000, behavior: "smooth" });
    ...
    $(document).on("click", "#user_list", function (e) {
      $(
        ".hk-chat-popup header > .input-group,.hk-chat-popup .contact-list-wrap"
      ).removeClass("d-none");
      $(
  ...
  if ($(".hk-chatbot-popup .chatbot-popup-body").length > 0) {
    var demoScroll = document.querySelector(
      ".chatbot-popup-body .simplebar-content-wrapper"
    );
    demoScroll.scrollTo({ top: 100000, behavior: "smooth" });
    $(document).on("click", ".btn-popup-open", function (e) {
    ...
    $(document).on("click", "#minimize_chatbot", function (e) {
      $(".btn-popup-close")
        .addClass("btn-popup-open")
        .removeClass("btn-popup-close");
      $(".hk-chatbot-popup").removeClass("d-md-block d-flex");
      return false;
    });
    $(document).on("keypress", "#input_msg_chat_popup", function (e) {
...
var emailApp = function () {
  ...
  $(document).on("click", "#min_compose_popup", function (e) {
    $(".compose-email-popup")
      .removeClass("maximize-email-popup")
      .toggleClass("minimize-email-popup");
    $wrapper.removeClass("hk__email__backdrop");
    return false;
  });
  $(document).on("click", "#max_compose_popup", function (e) {
    $(".compose-email-popup")
      .removeClass("minimize-email-popup")
      .toggleClass("maximize-email-popup");
    $wrapper.toggleClass("hk__email__backdrop");
    return false;
  });
  $(document).on("click", "#close_compose_popup", function (e) {
    $(".compose-email-popup").removeClass(
      "d-block maximize-email-popup minimize-email-popup"
    );
    $wrapper.removeClass("hk__email__backdrop");
    return false;
  });
...
var contactApp = function () {
  ...
  $(document).on(
    "click",
    ".contact-card-view input[type='checkbox']",
    function (e) {
...
var invoiceApp = function () {
  ...
  $(document).on(
    "click",
    ".invoiceapp-setting-toggle,.info-close",
    function (e) {
...
var galleryApp = function () {
  ...
  $(document).on(
    "click",
    ".galleryapp-wrap input[type='checkbox']",
    function (e) {
...
var taskboardApp = function () {
  ...
  $(document).on(
    "click",
    ".taskboardapp-info-toggle,.info-close",
    function (e) {
...
var todoApp = function () {
  ...
  $(document).on(
    "click",
    ".todoapp-wrap .edit-task,.todoapp-wrap .view-task,.todoapp-wrap .todo-text",
    function (e) {
...
$(window).on("resize", function () {
  ...
  /*Sticky  Header*/
  $(".header-sticky,.header-sticky thead tr th").css(
    "top",
    $navbar.outerHeight()
  );
